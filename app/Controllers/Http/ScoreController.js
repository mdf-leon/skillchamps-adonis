...
class ScoreController {
  ...
  async show({ request, response, auth, params }) {
    let score = await Score.query().where({ id: params.id })
  ...
  async destroy({ request, response, auth, params }) {
    ...
    let old = await Score.query().where({ id: params.id })
      .with('penalties')
      .with('bonuses')
      .first();
    if (old) {
      ...
      return response.status(400).json({not_found: 'could not found this score in DB'})
    }
  ...
  async index({ request, response, auth, params }) {
    ...
    if (trial_id || rider_id) {
      score = await Score.query().where(whereQuery)
        .with('penalties.penaltyConf')
        .with('bonuses.bonusConf')
        .fetch();
    } else {
  ...
  async store({ request, response, auth }) { // to create a score
